...
public class SpectateCycle implements CommandExecutor, TabCompleter {
        ...
        switch(args[0].toLowerCase()) {
            ...
            case "stop" -> {
                ...
                if(args.length == 1) {
                    ...
                    plugin.getCycleHandler().stopRunningCycle(player);
                ...
                if(hasPermission(player, COMMANDS_CYCLE_STOP_OTHERS)) {
                    ...
                    if(!spectateUtils.isCycling(target)) {
                        player.sendMessage(Messages.getMessage(Paths.MESSAGES_COMMANDS_CYCLE_TARGET_NOT_CYCLING, "TARGET", target.getName()));
                        return true;
                    }

                    plugin.getCycleHandler().stopRunningCycle(target);
    ...
    private void startSpectateCycle(Player player, int interval, boolean alphabetical) {
        ...
        plugin.getCycleHandler().startNewCycle(player, interval, false, alphabetical);
    }
